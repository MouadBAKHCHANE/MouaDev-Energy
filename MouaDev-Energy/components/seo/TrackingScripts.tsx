...
'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
      ...
      {/* Google Analytics 4 (only if GTM is not used — GTM handles GA internally) */}
...
gtag('js',new Date());gtag('config','${googleAnalyticsId}');`}
      ...
      {googleAdsId && !googleTagManagerId && (
        <>
          <Script
...
export function TrackingBodyStart({
  googleTagManagerId,
  bodyStartScripts,
}: TrackingScriptsProps) {
  return (
...
export function TrackingBodyEnd({ bodyEndScripts }: TrackingScriptsProps) {
  if (!bodyEndScripts) return null
  return <div dangerouslySetInnerHTML={{ __html: bodyEndScripts }} />
}
